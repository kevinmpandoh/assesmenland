...
-- ============================================================
-- Write policies for the trusted server functions.
--
-- Lovable Cloud does not expose a service-role key to the server runtime —
-- only the project URL + publishable (anon) key — so the server writes with
-- the anon role, which RLS applies to. All writes still flow exclusively
-- through createServerFn handlers (validated, rate-limited); the browser
-- never gets a write path because it has no direct table access in the UI.
-- These policies let those server writes land in the shared database so the
-- leaderboard and player progress survive redeploys. (drop+create keeps this
